 ...
 * <li><b>identifyTemplates</b> - Esse parâmetro define se, para cada captura, será verificada se o template é compatível com os templates armazenados internamente.</li>
 * <li><b>identifyThreshold</b> - Esse parâmetroc define o limiar que determina quando ocorre uma verifica&ccção com sucesso entre dois templates. Por padrão é <b>20</b></li>
 * </ul>
 * 
 * @author <a href="mailto:moacyr.ricardo@veridistec.com.br">Moacyr Ricardo Pereira Neto</a>
 * 
 */
public class AppletIdentificationSample extends JApplet implements CaptureEventListener {
	...
	public void init() {
		...
		try {
			InputStream in = AppletIdentificationSample.class.getResourceAsStream("veridislogo.png");
			if (in == null) 
	 ...
	 * 
	 * @see AppletIdentificationSample#persistTemplateDataOnTemplates()
	 */
	...
	/**
	 * This method is the counterpart of the method <i>{@link AppletIdentificationSample} .startCapture()</i> 
	 * and causes the applet to stop responding to events of the capture device.<br/>
	 * <br/>
	 * Esse método é a contrapartida do método <i>{@link AppletIdentificationSample} .startCapture()</i> 
	...
	public void onCaptureEvent(CaptureEventType evt, BiometricScanner bioScanner, BiometricImage img) {
		...
		if (evt == CaptureEventType.IMAGE_CAPTURED) {
			...
			boolean identifyTemplates    = Boolean.parseBoolean( getParameter("identifyTemplates"   , "false") );
			...
			if (verifyTemplates) {
				...
				if(identifyTemplates){
					int     identifyThreshold = Integer.parseInt    ( getParameter("identifyThreshold", "25") );
					captureFailed = false;
					BiometricIdentification bioId = new BiometricIdentification(template);
					identificationScore = -1;
					identificationIndex = -1;
					for (int ind = 0;ind < allTemplates.size();ind++){
						BiometricTemplate tpt = allTemplates.get(ind);
						int score = tpt.match(template);
						if (score >= identifyThreshold){
							debug("Identified: score = " + score + "; min = " + identifyThreshold+";index="+(ind+1));
